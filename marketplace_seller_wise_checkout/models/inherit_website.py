...
class Website(models.Model):
    ...
    def _get_all_seller_sale_order_ids(self):
        seller_so_ids = request.session.get('seller_so_ids') or False
        seller_so_ids = request.env['sale.order'].sudo().browse(seller_so_ids).exists()
        # if request.env.user != request.website.user_id and not seller_so_ids:
        if request.env.user != request.website.user_id:
            partner_id = request.env.user.partner_id
            seller_so_ids = partner_id.sale_order_ids.filtered(lambda o: o.state == 'draft' and o.marketplace_seller_id.id != False)
    ...
    def sale_get_order(self, force_create=False, code=None, update_pricelist=False, force_pricelist=False, seller_id=False):
        ...
        request.session['seller_so_ids'] = self._get_all_seller_sale_order_ids().ids if self._get_all_seller_sale_order_ids() else None
        if seller_id:
            ...
            if seller_so_id:
                ...
                # Update the last website so_id
                partner.last_website_so_id = seller_so_id if last_order and last_order.marketplace_seller_id else last_order
                if not last_order:
                    partner.last_website_so_id = seller_so_id
                return request.env['sale.order'].sudo().browse(seller_so_id.id)

        # Update the last website so_id
        if not seller_id:
            last_order = self.env.user.partner_id.last_website_so_id
            if last_order and last_order.marketplace_seller_id:
        ...
        # Update the last website so_id
        if not seller_id:
            admin_last_order = self.env.user.partner_id.last_website_so_id
            if admin_last_order and not admin_last_order.marketplace_seller_id:
                self.env.user.partner_id.last_website_so_id = admin_last_order
            else:
                self.env.user.partner_id.last_website_so_id = last_order
