...
class website_account(website_account):
    ...
    def addr_values_postprocess(self, addrMode, preValues, errors, errorMsg):
        newValues = {}
        authorizedFields = request.env['ir.model'].sudo().search(
            [('model', '=', 'res.partner')])._get_form_writable_fields()
        for k, v in preValues.items():
            # don't drop empty value, it could be a field to reset
            if k in authorizedFields and v is not None:
                newValues[k] = v
            else:  # DEBUG ONLY
        ...
        if 'street2' not in newValues and preValues.get('street2'):
            newValues['street2'] = preValues.get('street2')
    ...
    def my_address(self, **kw):
        ...
        if 'submitted' in kw:
            ...
            if errors:
                ...
                partner_id = self._addr_form_save(addrMode, postValues, kw)
                return request.redirect('/my/addressbook')
