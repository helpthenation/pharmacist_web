...
class MarketplaceStock(models.Model):
    ...
    # Used In marketplace 9 but now is depricated
    @api.model
    def _set_seller_only(self):
        seller_group = self.env['ir.model.data'].get_object_reference(
            'odoo_marketplace', 'marketplace_seller_group')[1]
    ...
    # def product_temp_id_change(self):
    #     result = {}
    #     self.ensure_one()
    #     if self.product_temp_id:
    #         self.product_id = self.product_temp_id.product_variant_ids[0].id
    #         self.location_id = self.product_temp_id.marketplace_seller_id.location_id.id or False,
