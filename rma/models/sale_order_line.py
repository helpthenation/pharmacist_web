...
class SaleOrderline(models.Model):
    ...
    def _delivery_state(self):
        for line in self:
            if line.procurement_ids:
                config_res = self.env[
                    "rma.config.settings"].get_default_rma_setting_fields()
                line.delivery_state = all([proc.state in ['done'] for proc in line.procurement_ids])
                # if config_res["process_do_state"] == "all":
                #     line.delivery_state = all([proc.state in ['draft', 'waiting', 'cancel', 'confirmed',
                #                                               'partially_available', 'assigned', 'done'] for proc in line.procurement_ids])
                # if config_res["process_do_state"] == "done":
                #     line.delivery_state = all(
                #         [proc.state in ['done'] for proc in line.procurement_ids])
            else:
    ...
    def _check_eligibility_for_rma(self):
        config_setting = self.env[
            "rma.config.settings"].get_default_rma_setting_fields()
        for obj in self:
            if config_setting.has_key("days_for_rma"):
                create_date = None
                if config_setting.has_key("rma_day_apply_on") and config_setting["rma_day_apply_on"] == "so_date":
                    create_date = datetime.strptime(
                        obj.create_date, '%Y-%m-%d %H:%M:%S').date()

                if config_setting.has_key("rma_day_apply_on") and config_setting["rma_day_apply_on"] == "do_date":
                    move_line = self.env["stock.move"].search(
                        [("product_id", "=", obj.product_id.id), ("origin", "=", obj.order_id.name)])

                    if move_line and move_line[0].picking_id.date_done:
                        done_date = datetime.strptime(
                            move_line[0].picking_id.date_done, '%Y-%m-%d %H:%M:%S').date()
    ...
    def get_rma_t_and_c(self):
        x = self.env["rma.config.settings"].get_default_rma_setting_fields(
...
class SaleOrder(models.Model):
    ...
    def get_rma_sale_setting(self):
        return self.env["rma.config.settings"].get_default_rma_setting_fields()
