...
class RmaRma(models.Model):
    ...
    def _get_term_condition(self):
        res = self.env['rma.config.settings'].get_default_rma_setting_fields()
        rma_term_condition = ""
        if res.has_key("rma_term_condition"):
            rma_term_condition = res["rma_term_condition"]
...
# Advance Features
    ...
    def get_request_type(self):
        ...
        vals = irmodule_obj.sudo().search([('name', 'in', ['mrp_repair']), ('state', 'in', [
    ...
    product_id = fields.Many2one(
        'product.product', related='orderline_id.product_id', string='Product', required=True, track_visibility='onchange', store=True)
    refund_qty = fields.Integer(
        'Refund Quantity', required=True, track_visibility='onchange')
...
# Advance Fields
    ...
    def view_repair_job(self):
        ...
        mrp_repair_id = self.env["mrp.repair"].browse(int(self.mrp_repair_id))
    ...
    def create(self, vals):
        if vals.get('name', 'New RMA') == 'New RMA':
            vals['name'] = self.env['ir.sequence'].next_by_code(
                'rma.rma') or 'New RMA'
        if vals.get("refund_qty", 0) <= 0:
            raise UserError(_('You can not create RMA with return quantity 0 .'))

        sale_order_line_obj = self.env["sale.order.line"].browse(vals["orderline_id"]) if vals.has_key("orderline_id") else False
        ...
        net_refund_qty = rma_refund_qty + vals.get('refund_qty', 0)
    ...
    def get_show_rma_stage_value(self):
        res = self.env['rma.config.settings'].get_default_rma_setting_fields()
        if res.has_key("show_rma_stage") and res["show_rma_stage"]:
            return True
        else:
            return False
