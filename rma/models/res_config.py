...
class RmaConfigSettings(models.TransientModel):
    ...
    process_do_state = fields.Selection(selection, string="Process state")
    ...
    module_mrp_repair = fields.Boolean(
    ...
    @api.one
    def set_default_rma_setting_fields(self):
        ir_values = self.env['ir.values']

        self.env['ir.values'].sudo().set_default(
            'rma.config.settings', 'allow_quote_cancellation', self.allow_quote_cancellation)
        self.env['ir.values'].sudo().set_default(
            'rma.config.settings', 'process_do_state', self.process_do_state)
        self.env['ir.values'].sudo().set_default(
            'rma.config.settings', 'rma_term_condition', self.rma_term_condition)
        self.env['ir.values'].sudo().set_default(
            'rma.config.settings', 'days_for_rma', self.days_for_rma)
        self.env['ir.values'].sudo().set_default(
            'rma.config.settings', 'rma_day_apply_on', self.rma_day_apply_on)
        self.env['ir.values'].sudo().set_default(
            'rma.config.settings', 'repair_location_id', self.repair_location_id.id)
        self.env['ir.values'].sudo().set_default(
            'rma.config.settings', 'show_rma_stage', self.show_rma_stage)
        return True

    @api.model
    def get_default_rma_setting_fields(self, fields=None):
        ir_values = self.env['ir.values']
        allow_quote_cancellation = self.env['ir.values'].get_default(
            'rma.config.settings', 'allow_quote_cancellation')
        process_do_state = self.env['ir.values'].get_default(
            'rma.config.settings', 'process_do_state')
        rma_term_condition = self.env['ir.values'].get_default(
            'rma.config.settings', 'rma_term_condition') or "About Return and Refund Policies Most e-commerce stores should have a Return or Refund Policy, just as it should have a Privacy Policy. Wikipedia defines Returning as: In retail, returning is the process of a customer taking previously purchased merchandise back to the retailer, and in turn, receiving a cash refund, exchange for another item (identical or different), or a store credit. Most countries industry regulations require that stores (even digital) must have this kind of policy. eBay’s help pages mention that stores with return policies published online sell better (however, eBay requires all stores to have this policy): We’ve found that items with clear return policies typically sell better than items that don’t. A Terms and Conditions agreement for your store might be a good idea, but it is not required by law. If you’re looking for a Terms and Conditions agreement, you can generate it with our generator. What to include in your policy Your Return Policy should include at least the following sections: the numbers of days a customer can notify you for wanting to return an item after they received it what kind of refund you will give to the customer after the item is returned: another similar product, a store credit, etc. who will pay for the return shipping"
        days_for_rma = self.env['ir.values'].get_default(
            'rma.config.settings', 'days_for_rma')
        rma_day_apply_on = self.env['ir.values'].get_default(
            'rma.config.settings', 'rma_day_apply_on') or "do_date"
        show_rma_stage = self.env['ir.values'].get_default(
            'rma.config.settings', 'show_rma_stage')
        repair_location_id = self.env['ir.values'].get_default(
            'rma.config.settings', 'repair_location_id')
        return {
